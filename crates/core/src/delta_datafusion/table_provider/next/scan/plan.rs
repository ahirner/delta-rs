...
use datafusion::common::{HashSet, exec_err};
...
impl KernelScanPlan {
    pub(crate) fn try_new(
        ...
        let (kernel_predicate, parquet_predicate) =
            super::filter::process_filters(filters, table_config, config)?;
        ...
        let scan_predicate = if let Some(sp) = skipping_predicate {
            let (Some(pred), _) = super::filter::process_filters(&sp, table_config, config)? else {
